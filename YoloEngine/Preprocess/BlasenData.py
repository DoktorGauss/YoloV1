...
class blasen_set(object):
    def __init__(self, rebuild=False):
        self.devkil_path = os.path.join(cfg.BLASEN_PATH, '')
    ...
    def load_labels(self, model):
        if model == 'train':
            self.devkil_path = os.path.join(cfg.PASCAL_PATH, '')
            self.data_path = os.path.join(self.devkil_path, 'train')
            txtname = os.path.join(self.data_path, 'trainval.txt')
        if model == 'test':
            self.devkil_path = os.path.join(cfg.PASCAL_PATH, '')
            self.data_path = os.path.join(self.devkil_path, 'test')
        ...
        for index in self.image_index:
            ...
            if num == 0:
                continue
            imname = os.path.join(self.data_path, index + '.jpg')
    ...
    def load_pascal_annotation(self, index):
        ...
        filename = os.path.join(self.data_path, index + '.xml')
