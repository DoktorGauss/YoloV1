...
y_val = tf.reshape(y_val[index], (-1, S[0],S[1], (5*B+C)))
val_scores, val_boxes, val_classes = get_boxes(tf.cast(y_val, dtype='float'), S, B, yolo_input, 0.05)
val_boxes = kb.reshape(val_boxes, (1,-1,4))
val_colors = np.array([[0.0, 1.0, 0.0]])
myTrueBoxImage = tf.image.draw_bounding_boxes([images[index]], val_boxes,colors=val_colors)
plt.imshow(myTrueBoxImage[0])
plt.show()